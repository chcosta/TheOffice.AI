# TheOffice.AI — Features & Reference

A deep dive into what TheOffice.AI is and everything it can do. For install and a quick
overview, see the [main README](../README.md). For the latest changes, see the
[release notes](https://github.com/chcosta/TheOffice.AI/releases).

---

## What it is

An intelligent orchestration platform for GitHub Copilot CLI agents. Schedule, monitor,
and chain AI agents together — with **Managers** that coordinate multi-agent workflows,
real-time chat, cloud sync, and a full web dashboard. Every agent runs **locally, on your
machine, with your own credentials**.

The browser and desktop apps share the exact same `server.js` + `public/app.html` — the
desktop shell (Tauri v2) just wraps them and runs the server as a local sidecar.

---

## Screenshots

**Board — Pin view (focus mode):** a free-form, spatial workspace. Pin sessions, chats,
tasks, agents, notes, and live **dev cards**; an always-on AI **Briefing** keeps you
oriented on where you left off.

![Board pin view](board-pin-view.webp)

**Board — Tabbed view:** the same pinned items rendered as a compact tab rail with a
focused detail pane, with the Briefing surfacing deltas since you last looked.

![Board tabbed view](board-tabbed-view.webp)
...
![Manager Detail](manager-detail.png)
