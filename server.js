...
app.post('/api/boards/:id/ai-layout', async (req, res) => {
  ...
  const sys = [
    ...
    '- width: optional "full" (span the whole board width — use ONLY for the single most important / widest-content panel, e.g. a substantive briefing or a long note) or "half" (the default column width). Use "full" sparingly (at most one or two panels) — the board arranges open "half" panels into TWO balanced side-by-side columns, so prefer "half" for most panels to keep the layout compact and symmetrical.',
    '- DESIGN GOAL: produce a compact, visually balanced view with minimal wasted space — collapse low-signal panels aggressively so the open ones pack tightly, keep roughly even amounts of content open (the two columns balance automatically), and pick a zoom that fills the viewport without large empty gaps.',
