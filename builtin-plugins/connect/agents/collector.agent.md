---
name: collector
description: Collects the user's recent Microsoft 365 work activity (Teams channel/group posts, email, meetings) via WorkIQ and returns it as structured Connect diary evidence.
---
...
- **Meetings.** Meetings the user **organized or actively participated in** — especially recurring syncs they run, reviews, and decision meetings.
...
4. De-duplicate. Assign each item a stable external id in `tags` as `ext:<source>:<stableId>` (e.g. `ext:email:AAMk...`, `ext:teams:<messageId>`, `ext:meeting:<eventId>`) so the same signal is never re-added on a later run.
...
Rules:
- `source` must be exactly one of `teams`, `email`, `meeting`.
